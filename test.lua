...
local aa = {
    function()
        ...
        -- ToggleButton: Frame ImageButton để bật/tắt GUI
        x.ToggleButtonVisible = true
        x.GUIVisible = true

        function x.CreateToggleButton(C, D)
            D = D or {}
            local btnSize = D.Size or UDim2.new(0, 50, 0, 50)
            local btnPos = D.Position or UDim2.new(1, -60, 0.5, -25)
            local btnImage = D.Image or "rbxassetid://10723407389"
            local btnColor = D.Color or Color3.fromRGB(30, 30, 35)
            local btnImageColor = D.ImageColor or Color3.fromRGB(255, 255, 255)
            local cornerRadius = D.CornerRadius or UDim.new(0, 12)

            -- Outer Frame container
            local toggleFrame = u("Frame", {
                Name = "ToggleButtonFrame",
                Size = btnSize,
                Position = btnPos,
            ...
            -- Shadow / glow effect frame
            local shadowFrame = u("Frame", {
                Name = "Shadow",
            ...
            u("UICorner", {CornerRadius = UDim.new(0, cornerRadius.Offset + 4), Parent = shadowFrame})

            -- Main ImageButton
            local imageButton = u("ImageButton", {
                ...
                BorderSizePixel = 0,
                Image = btnImage,
                ImageColor3 = btnImageColor,
                ScaleType = Enum.ScaleType.Fit,
                ZIndex = 10,
                Parent = toggleFrame
            })
            u("UICorner", {CornerRadius = cornerRadius, Parent = imageButton})
            u("UIStroke", {
            ...
            u("UICorner", {CornerRadius = cornerRadius, Parent = hoverOverlay})

            -- Tooltip label
            local tooltip = u("TextLabel", {
                Name = "Tooltip",
                Text = x.GUIVisible and "Ẩn GUI" or "Hiện GUI",
                Size = UDim2.new(0, 80, 0, 24),
                Position = UDim2.new(-1.6, 0, 0.5, -12),
                AnchorPoint = Vector2.new(0, 0),
            ...
            u("UICorner", {CornerRadius = UDim.new(0, 6), Parent = tooltip})
            u("UIPadding", {PaddingLeft = UDim.new(0, 6), PaddingRight = UDim.new(0, 6), Parent = tooltip})

            x.ToggleButton = imageButton
            x.ToggleFrame = toggleFrame

            -- Hover animation
            imageButton.MouseEnter:Connect(function()
                l:Create(hoverOverlay, TweenInfo.new(0.15), {BackgroundTransparency = 0.88}):Play()
                l:Create(imageButton, TweenInfo.new(0.15), {ImageTransparency = 0.1}):Play()
                tooltip.Text = x.GUIVisible and "Ẩn GUI" or "Hiện GUI"
                tooltip.Visible = true
            end)
            imageButton.MouseLeave:Connect(function()
                l:Create(hoverOverlay, TweenInfo.new(0.15), {BackgroundTransparency = 1}):Play()
                l:Create(imageButton, TweenInfo.new(0.15), {ImageTransparency = 0}):Play()
                tooltip.Visible = false
            end)

            -- Click: toggle GUI visibility
        ...
        return function(t)
            ...
            function v.Minimize(M)
                ...
                if not C then
                    ...
                    u:Notify{Title = "Interface", Content = "Press " .. N .. " to toggle the inteface.", Duration = 5}
                end
            end
